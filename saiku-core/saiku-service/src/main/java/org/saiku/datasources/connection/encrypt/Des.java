...
public final class Des {
  ...
  /**
   * ********************************************************************************************************************************
   * Perform an encryption/decryption operation, which being determined by how the keys were specified. The length must
   * be a multiple of the block size - 64 bits or 8 bytes. Anything else is reject, so it is up to the caller to manage
   * that.
   * <p/>
   * Parameters: pvTo - destination pvFrom - source, may be the same as pvTo. cb - number of bytes to transform,
   * multiple of 8. Returns: 0 if size is inappropriate, else all is well, conversion done. ---
   */
  int Crypt(byte[] pvTo,
            @NotNull byte[] pvFrom) {
  ...
  public int Crypt(@NotNull byte[] pv) {
    return Crypt(pv,
        pv);
  }
  ...
  /**
   * Generate the keys used during each round. There are several variations, depending upon encryption/decryption key,
   * and which of the 2 possible keys. The keys are passed in, solving the latter problem.
   *
   * @param pRound  the array to fill with the transformed keys
   * @param pKey    the key to use, in internal 2 x 28 bit format
   * @param encrypt <code>true</code> for encryption, <code>false</code> for decryption
   */
